...
def test_fun1(source, params):
    ...
    for i in range(defocus_ct):
        t1 = time.time()
        images = fast_pca.src.projections[(i*batch_size):min(((i+1)*batch_size),N)].asnumpy()*img_size
        # denoise_options = {
        #     "denoise_df_id": [i],
        ...
        # images = results["clean_images"]*img_size
...
def test_fun2(source, params):
    ...
    for i in range(defocus_ct):
        t1 = time.time()
        images0 = fast_pca.src.projections[(i*batch_size):min(((i+1)*batch_size),N)].asnumpy()*img_size
        denoise_options = {
            "denoise_df_id": [i],
            "denoise_df_num": [batch_size],                            
            "return_denoise_error": True,
        ...
        results = fast_pca.denoise_images(mean_est=mean_est, 
                                          covar_est=covar_est, 
                                          denoise_options=denoise_options)
        images = np.array(results["clean_images"], dtype=np.float32)*img_size
        print(LA.norm(images0.flatten()-images.flatten())/LA.norm(images.flatten()))
        images = image_downsample(images, ds_res, True)
