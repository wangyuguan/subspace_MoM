...
with mrcfile.new('offsetimage_test/vol_ds.mrc', overwrite=True) as mrc:
...
offsets = np.random.normal(0,1,(N,2))
offset_scale = np.linspace(0,10,11,endpoint=True)

# %% create image simulation 
noise_adder = WhiteNoiseAdder(var=0)
for j in range(len(offset_scale)):
    
    offsets_j = offsets*offset_scale[j]
    source = Simulation(
        L=img_size,
        n=N,
        vols=Vol,
    ...
    # save results
    filename = 'offsetimage_test/res2_'+str(j)+'.mat'
    savemat(filename,res2)
    filename = 'offsetimage_test/res3_'+str(j)+'.mat'
    savemat(filename,res3)
    ...
    filename = 'offsetimage_test/vol_est_m2_'+str(j)+'.mrc'
    with mrcfile.new(filename, overwrite=True) as mrc:
        mrc.set_data(vol_est_m2.reshape([ds_res,ds_res,ds_res]))  # Set the volume data
        mrc.voxel_size = 1.0  

    filename = 'offsetimage_test/vol_est_m3_'+str(j)+'.mrc'
    with mrcfile.new(filename, overwrite=True) as mrc:
    ...
    filename = 'offsetimage_test/vol_est_m2_aligned_'+str(j)+'.mrc'
    with mrcfile.new(filename, overwrite=True) as mrc:
        mrc.set_data(vol2_aligned[0])  # Set the volume data
        mrc.voxel_size = 1.0  

    filename = 'offsetimage_test/vol_est_m3_aligned_'+str(j)+'.mrc'
    with mrcfile.new(filename, overwrite=True) as mrc:
